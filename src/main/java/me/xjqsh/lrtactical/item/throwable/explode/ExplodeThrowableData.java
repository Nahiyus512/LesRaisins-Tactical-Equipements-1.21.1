...
public class ExplodeThrowableData extends ThrowableData {
    ...
    public List<TooltipLine> getTooltipLines() {
        ...
        ExplodeData explode = getExplode();
        lines.add(TooltipLine.normal(Component.translatable("tooltip.lrtactical.throwable.explode.line",
                TooltipUtil.format((float) explode.getDamage()),
                TooltipUtil.format(explode.getRadius()))));
        ...
        if (!traits.isEmpty()) {
            ...
            for (int i = 1; i < traits.size(); i++) {
                joined.append(Component.literal(", ")).append(traits.get(i));
            }
            lines.add(TooltipLine.normal(Component.translatable("tooltip.lrtactical.throwable.explode.traits", joined)));
        }
        return lines;
    }
